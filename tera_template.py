...
class TERAReportGenerator:
    ...
    def generate(self) -> str:
        ...
        logo_option = self.data.get("logo_option", "without_logo")
        logo_path = os.path.join(os.path.dirname(__file__), "logo.png")

        # PAGE 1
        if logo_option == "with_logo":
            self._draw_custom_header(c, logo_path)
    ...
    def _draw_custom_header(self, c, logo_path):
        header_height = 70
        ...
        if os.path.exists(logo_path):
            img_width = W - 220   # full width with margins
            img_height = 50
            c.drawImage(
                logo_path,
                40,
    ...
    # Divider line
        c.setLineWidth(1)
        c.line(30, H - header_height, W - 30, H - header_height)
    # ═══════════════════════════════════════════════════════════════════════════
    # Shared header / footer
    # ═══════════════════════════════════════════════════════════════════════════
    def _header(self, c):
        # ❌ Disable old header if using custom logo
        if self.data.get("logo_option") == "with_logo":
            return
